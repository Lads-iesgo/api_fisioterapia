...
// ─── 1. Verifica se o usuário tem um dos perfis permitidos ───────────────────────────
...
// ─── 2. Permite apenas GET para Aluno/Fisioterapeuta; outros passam livremente ──────
export const readOnlyForStudents = (req: Request, res: Response, next: NextFunction) => {
  const user = req.user;
  if (!user) {
    res.status(401).json({ message: "Não autenticado." });
    return;
...
// ─── 3. Valida propriedade de Consulta para Aluno/Fisioterapeuta ──────────────────
...
// ─── 4. Restringe Professor a criar apenas usuários com perfis "aluno" ou "fisioterapeuta" ────
export const restrictProfessorUserCreation = async (
  ...
  if (user.role === ROLES.PROFESSOR) {
    ...
    const idsPermitidos = perfilsPermitidos.map((p) => p.id);

    if (!idsPermitidos.includes(perfil_id)) {
      res.status(403).json({
        message: "Professores só podem criar usuários com perfil de Aluno ou Fisioterapeuta.",
      });
      return;
    }
  }
...
// ─── 5. Aplica isolamento de dados para leitura de Consultas para Aluno/Fisioterapeuta ──
export const applyConsultaDataIsolation = (req: Request, res: Response, next: NextFunction) => {
  const user = req.user!;
